...
author: "Andreina Arias, Nakesha Fray,Yanyi Li, Leslie Tavarez and Jiaxin Zheng"
date: "2024-10-26"
...
final_df$skill <- str_trim(final_df$skill)  
...
```{r}
program_lang <- c("sql", "java", "python","r", "spark", "pandas", "sas", "c++", "c/c++")
...
ggplot(pro_lang_df, aes(x = reorder(skill, -n), y=n)) + 
...
#Data tools ggplot
ggplot(data_tool_df, aes(x = reorder(skill, -n), y=n)) + 
...
#Data Skill ggplot
ggplot(data_skill_df, aes(x = reorder(skill, -n), y=n)) + 
...
ggplot(data_visual_df, aes(x = reorder(skill, -n), y=n)) + 
